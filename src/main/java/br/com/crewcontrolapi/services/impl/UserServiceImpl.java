package br.com.crewcontrolapi.services.impl;
...
import br.com.crewcontrolapi.domain.pagination.CustomPageImpl;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    @Transactional(readOnly = true)
    public CustomPage<UserDTO> getUsers(Pageable pageable, User requestingUser) {
        Page<User> userPage;

        if (requestingUser.getRole().equals(RoleEnum.ADMINISTRATOR)) {
            userPage = userRepository.findNonAdminUsers(pageable);
        } else if (requestingUser.getRole().equals(RoleEnum.LEADER)) {
            userPage = userRepository.findCollaboratorsByTeamId(requestingUser.getTeam().getId(), pageable);
        } else {
            userPage = userRepository.findByUserId(requestingUser.getId(), pageable);
        }
        Page<UserDTO> dtoPage = userPage.map(UserMapper::toDTO);
        return new CustomPageImpl<>(dtoPage);
    }
